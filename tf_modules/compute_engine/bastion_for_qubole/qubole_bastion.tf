...
resource "google_compute_instance" "qubole_bastion_host" {
    ...
    network_interface {
        ...
        network_ip = google_compute_address.qubole_bastion_internal_ip.address
        access_config {
            name: 'External NAT',
            type: 'ONE_TO_ONE_NAT',
            natIP: google_compute_address.qubole_bastion_host_external_ip.address
        }
    }
